...
m2 <- matrix(c(12:24), nrow = 2, ncol = 4, byrow = TRUE) # If you pass more values or fewer values that either be more for the given rows and columns or less, it will throw an warning. 

class(m1)
dim(m1)

# When you want to add two or more matrices, It is necessary that the matrices are of the same dimension. 
m1 + m2 # Both are different dimensions, so it will throw an error. 
m3 <- matrix(c(20:30), nrow =4, ncol = 2)
m1 + m3 # Both are of the same dimensions

# TO do the dot product (Matrix multiplication), the no of the col of the first matrix should be equal to the rows of the second matrix
...
n3 <- m1[,c(1:2)] # Here we are extracting all rows and the two cols. 
class(n3) # Now the class of n3 will be Matrix or array


####################### Arrays ##################################

# Array is as same as a matrix but it is a multi-dimensional datastructure. 

arr1 <- array(c(1:20), dim = c(2,3,4)) # Here we are creating an array with the values 1 to 20 and the dimensions of 2 rows and 3 cols, and the no of matrixes in the array is 4. 
# when we want to pass multiple values we use c that is we are passing a vector. 
...
list1[1] # When we use this [] command, this will give the element of the list that is out is also the list. the class will be list
class(list1[1])

t <- list1[[1]] # Use the double squared brackets to get the exact date of the element, that is vector or integer in our case. 
class(t)
