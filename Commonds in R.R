...
class(heart_data) # Clase of our dataset
View(heart_data) # Displays the dataset
dim(heart_data) # gives the dimensions of the dataset

head(heart_data) # By default it will display only the first six rows
...
colnames(heart_data)[1] <- 'Patient Age'  # Changin the first col name


# Unique values in all col
heart_data$ChestPainType  # use $ to get the data of that col.
unique(heart_data$ChestPainType) # unique value of chestpainttype col, length will give the length of all unique values. 
